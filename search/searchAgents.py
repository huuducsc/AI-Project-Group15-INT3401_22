    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                new_state = (next_position, visited_corners)
...
def cornersHeuristic(state, problem):
    ...
    (position, visited_corners) = state
    remain_corners = list(set(corners).difference(set(visited_corners)))
    ...
    while (len(remain_corners) > 0):
        ...
        new_position = remain_corners[0]
        for corner in remain_corners:
            if nearest_distance > manhattan_distance(position, corner):
                nearest_distance = manhattan_distance(position, corner)
                new_position = corner
        position = new_position
        totalcost += nearest_distance
        remain_corners.remove(position)
    return totalcost
